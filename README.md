...
步驟 1.a：在高位回落時買入認跌期權（Long Put） [1]
* 策略用意：在股價達到局部高點（Local Peak）並開始呈現下降趨勢時買入當月認跌期權（Put Option）。
* 目的：這是一個純粹的看跌投機，旨在利用股價下跌獲利。此時你尚未持有股票。 [1, 2, 3, 4, 5]
...
* 零成本心理：由於認跌期權已達損益平衡（甚至已獲利），這為你的現貨頭寸提供了緩衝空間。 [2, 6, 7, 8]
...
* 鎖定利潤區間：你設定了股票的上行獲利上限（認購期權行使價）與下行保護下限（認跌期權行使價）。 [1, 3, 8, 9, 10]
...
另外可以針對股票的波動率（IV），來計算這套策略在不同行使價下的損益圖表。
